"""Tests for `prpilot init` (bundled templates and regression of the empty-file generation bug)."""
...
def test_init_default_output_is_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli, "DEFAULT_CONFIG_PATH", home / ".prpilot.yaml")

    rc = cli.cmd_init(_args(None, force=False))
    assert rc == 0
    dest = home / ".prpilot.yaml"
    assert dest.is_file()
