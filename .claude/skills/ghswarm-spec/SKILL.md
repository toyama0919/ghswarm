...
- For a verify list that `cd`s into multiple directories, is each element written as a bare `cd <path> && <cmd>` **without parentheses** (ghswarm auto-wraps each element in a subshell; hand-writing `(...)` makes it `((...))`, which bash interprets as arithmetic evaluation and errors as a syntax error)?
