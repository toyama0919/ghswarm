---
name: ghswarm-spec
description: Line 1 of spec-driven development. Drafts a spec into tmp/spec/ from a feature request, chat YAML, or topic; goes through human review (approval) → an independent review by a subagent in a separate session triggered by that approval; then registers a GitHub Issue with the spec prose, task checklist, and GHSWARM_VERIFY block for ghswarm. Use for creating specs, writing specs, turning things into Issues, spec-driven work, and filing ghswarm tasks.
---
...
- Line 1 (this skill, human in the loop): **draft into tmp/spec/ → human review (approval) → AI review in a separate session triggered by that approval → create Issue with GHSWARM_VERIFY**
- Line 2 (ghswarm daemon, autonomous): cut branch `issue-<N>` → implement → AI review → create PR → wait for CI/approve → auto-merge

This skill **does not implement**. The goal is to prepare an approved spec and an Issue with a checklist of decomposed tasks. **No spec file is committed and no spec PR is opened** — the Issue body is the single source of the spec.

**Issue-as-single-source**: the spec prose, task checklist, and verify steps all live in the Issue body. ghswarm reads `GHSWARM_VERIFY` for verify and the body prose for implement/review prompts. Line 2 creates the branch and PR.
...
- Respect ghswarm's config (`target`, `branch_prefix`, `base_branch`).
...
- If `ghswarm config` exits non-zero (config not created, current repo not registered, `jq` absent, etc.), use these
  fallback defaults: `branch_prefix=issue-` / `base_branch=main` /
...
- `issue_create_args` — the array of flags to pass to `gh issue create` (already reflecting idle + target). **When creating, add `blocked_label` on top of these, and remove blocked at the very end after writing `GHSWARM_VERIFY`** (see steps 4-2/4-5; this avoids a race where the loop picks it up before verify metadata is written).

If `ghswarm config` exits non-zero, switch to the fallback defaults above.

**Determine the canonical spec language (`spec_lang`)**: whether the spec should stay in the draft's language in the Issue body, or be translated, is decided from **the target repository's `README.md` language** — record this as `spec_lang` (used in step 4-3). Read the top of `README.md` (title/overview) and take its dominant language as `spec_lang` (e.g. mostly Japanese → Japanese, mostly English → English). If there is no README or its language is unclear, look at the language of recent Issue bodies in the repository (open Issues filed by ghswarm, if any); if there are none either, **keep `spec_lang` as the draft's own language (no translation)**.
...
**Draft language**: write the `tmp/spec/` draft **in the author's native language** (Japanese for a Japanese speaker). The native language makes it easier to notice ambiguity in requirements, and the human review (step 2) is faster. **Do not translate at this stage** — whether it needs translating into `spec_lang` (determined in step 0, from the target repo's README) is decided all at once when you write the Issue body in step 4 (see step 4-3; no translation is needed at all if `spec_lang` matches the draft language).
...
### 4. Create/convert Issue → embed GHSWARM_VERIFY and GHSWARM_STATE → finally make it ready to start
Once you have both human approval (step 2) and passing the independent review (step 3), run the following in order.

**Important (race avoidance)**: the ghswarm loop **picks up Issues that match target and whose status is idle (not busy/blocked) on every poll**. If you create with idle before writing `GHSWARM_VERIFY` into the body, the loop can pick it up in the gap before writing and block it at the start gate for a missing spec (this actually happens). The fix is simple: **attach the blocked label at creation time, and remove it at the very end once `GHSWARM_VERIFY` is written**. While blocked is present, `is_locked` always excludes it (blocked takes precedence even if idle is also present), so the loop won't pick it up in the meantime.

**Do not cut a branch, commit a spec file, or open a spec PR.** ghswarm (Line 2) creates the branch and PR during implementation.

1. **Prepare the Issue body prose and verify block** from `tmp/spec/<slug>.md`:
   - **Compare the draft's language against `spec_lang`** (determined in step 0):
     - **They match**: use the draft body (without frontmatter) as-is.
     - **They differ**: translate the body into `spec_lang` without changing meaning. Keep `verify` commands, code blocks, checklist granularity, and functional keys intact.
   - **Transcribe the frontmatter `verify:`** into a `GHSWARM_VERIFY` HTML comment block (separate from `GHSWARM_STATE`, which ghswarm rewrites every iteration):
     ...
     The `verify` schema is unchanged (legacy string/list form, or list of `{path, command}`). **Verify command strings must not contain `-->` or `GHSWARM_VERIFY_END`** (HTML comment constraints). An empty `verify:` still counts as spec'd (local verification skipped).

   - Recommended body order: prose + task checklist → `GHSWARM_VERIFY` → `GHSWARM_STATE` (ghswarm always appends state last).

2. **Fix the Issue (with blocked)** and obtain the number N. The body is "background/purpose, requirements, design-approach summary" + the **task checklist** + **`GHSWARM_VERIFY`**. Branch into **A (create new) / B (convert existing issue)** by whether there is a source issue.
   ...
   **B. To convert an existing issue #N into ghswarm form** (conversion chosen in step 2). Use the source's number N. **First attach blocked by itself**, then set up the title, body, and target/idle labels (going idle first would let the loop pick it up in the gap, so always attach blocked first). Unlike create, `gh issue edit` uses `--add-label` / `--add-assignee`, so translate `issue_create_args`'s `--label`→`--add-label` and `--assignee`→`--add-assignee`:
   ...
   Rewrite the body **entirely** with the same content as A (reshape the rough original description into ghswarm form). Incorporate any information worth keeping from the original into the spec during drafting. If `ghswarm config` is unavailable, attach `--add-label "status: blocked"` first, then `--add-label "status: idle"` (+ target label/assignment per your setup). The subsequent steps (3–5) are common to A/B, using this N.

3. **Embed ghswarm's state metadata into the Issue body**. Append the following HTML comment to the end of the body and update via `gh issue edit <N> --body-file -`. Set `branch_name` to `<branch_prefix><N>` (ghswarm will create this branch during implement). Leave `pr_url` / `pr_number` empty — ghswarm creates the PR during Line 2.
...
4. **Delete the draft**: `rm -f tmp/spec/<slug>.md` (do not leave draft remnants in `tmp/spec/`).

5. **Finally, remove blocked to enable starting**. Only **at this point**, after finishing writing `GHSWARM_VERIFY` and `GHSWARM_STATE`, remove blocked. idle and target were already applied in step 4-2, so removing blocked leaves only idle and makes it ready to start:

   ```bash
   gh issue edit <N> --remove-label "$blocked"
   ```

   (If `ghswarm config` was unavailable, `gh issue edit <N> --remove-label "status: blocked"`.) **Do not do this blocked removal before writing `GHSWARM_VERIFY`** — that is the cause of the race.

### 5. Completion report and continuing development
Report the created Issue URL (**do not enumerate commands**).
...
## Notes
- Always match `branch_name` to `<branch_prefix><N>` (the `branch_prefix` obtained in step 0 + number). ghswarm creates this branch during implement.
- Set `next_action` to `start` (ghswarm starts from implement).
- Do not write to or commit spec files. Always keep the draft in `tmp/spec/` until filing, then delete it.
- Write the draft in the native language, and translate to `spec_lang` (the target repo's README language, determined in step 0) only if it differs from the draft language, when writing the Issue body in step 4. After writing a translated version, `rm -f` the native-language draft `tmp/spec/<slug>.md` so it does not remain in `tmp/spec/`.
